...
void FGameplayTagStackContainer::AddStack(FGameplayTag Tag, int32 StackCount)
{
	if (!Tag.IsValid())
	{
		FFrame::KismetExecutionMessage(TEXT("AddStack：GameplayTag 无效"), ELogVerbosity::Warning);
		return;
	}
...
void FGameplayTagStackContainer::RemoveStack(FGameplayTag Tag, int32 StackCount)
{
	if (!Tag.IsValid())
	{
		FFrame::KismetExecutionMessage(TEXT("RemoveStack：GameplayTag 无效"), ELogVerbosity::Warning);
		return;
	}

	//@TODO 移除不存在或层数不足时是否报错？
	if (StackCount > 0)
	{
