...
void SImpactProfileEditorViewport::UpdateDecalWireframe()
{
	...
	const uint8  DepthPriority = SDPG_Foreground; // 总是绘制在前面
	...
	LocalCorners[0] = FVector(-HalfSize.X, -HalfSize.Y, HalfSize.Z);  // 前左上
	LocalCorners[1] = FVector(-HalfSize.X, HalfSize.Y, HalfSize.Z);   // 前右上
	LocalCorners[2] = FVector(-HalfSize.X, -HalfSize.Y, -HalfSize.Z); // 前左下
	LocalCorners[3] = FVector(-HalfSize.X, HalfSize.Y, -HalfSize.Z);  // 前右下
	LocalCorners[4] = FVector(HalfSize.X, -HalfSize.Y, HalfSize.Z);   // 后左上
	LocalCorners[5] = FVector(HalfSize.X, HalfSize.Y, HalfSize.Z);    // 后右上
	LocalCorners[6] = FVector(HalfSize.X, -HalfSize.Y, -HalfSize.Z);  // 后左下
	LocalCorners[7] = FVector(HalfSize.X, HalfSize.Y, -HalfSize.Z);   // 后右下
