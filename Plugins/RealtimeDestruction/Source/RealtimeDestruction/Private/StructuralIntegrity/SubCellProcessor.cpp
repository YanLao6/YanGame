...
{
	// If the cell is completely destroyed, return 0
	...
	// If there is no SubCell state, all subcells are alive
	if (!SubCellState)
	{
		return SUBCELL_COUNT;
	}

	// Count the number of live subcells
	int32 Count = 0;
	for (int32 SubCellId = 0; SubCellId < SUBCELL_COUNT; ++SubCellId)
	{
		if (SubCellState->IsSubCellAlive(SubCellId))
...
{
	// If in DestroyedCells, it is completely destroyed
	if (CellState.DestroyedCells.Contains(CellId))
	{
		return true;
	}
	...
	// If there is no SubCell state, it is not destroyed yet
	return false;
}
...
TArray<int32> FSubCellProcessor::GetBoundarySubCellIds(int32 Direction)
{
	...
	// Determine the fixed axis based on the direction
...
uint32 FSubCellProcessor::GetBoundaryLiveSubCellMask(int32 CellId, int32 Direction, const FCellState& CellState)
{
	...
	for (int32 i = 0; i < BoundarySubCells.Num(); ++i)
	{
		...
		{
			bIsLive = true;  // If there is no SubCell state, all are alive
		}
...
{
	// mm -> cm conversion (multiply by 0.1)
	...
	case ECellDestructionShapeType::Box:
		{
			...
			{
				// With rotation: calculate AABB from 8 vertices
	...
	case ECellDestructionShapeType::Line:
		{
			// AABB of the line segment + thickness
			FBox Result(ForceInit);
			Result += Center;
			Result += EndPoint;
