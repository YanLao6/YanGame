...
void UNetworkTestSubsystem::ApplyPreset(ENetworkTestPreset Preset)
{
	if (const FNetworkTestPresetConfig* Config = PresetConfigs.Find(Preset))
	{
		...
		UE_LOG(LogNetworkTest,
		       Log,
		       TEXT("NetworkTestSubsystem: Applied preset '%s' (Lag:%dms Var:%dms Loss:%d%%)"),
		       *CurrentConfig.PresetName,
		       CurrentConfig.PktLag,
		       CurrentConfig.PktLagVariance,
		       CurrentConfig.PktLoss);
	}
}
...
void UNetworkTestSubsystem::ApplyCustomConfig(int32 PktLag, int32 PktLagVariance, int32 PktLoss)
{
	...
	UE_LOG(LogNetworkTest,
	       Log,
	       TEXT("NetworkTestSubsystem: Applied custom config (Lag:%dms Var:%dms Loss:%d%%)"),
	       PktLag,
	       PktLagVariance,
	       PktLoss);
}
...
{
	// UE 网络模拟 CVar 设置
	// Net.PktLag: 数据包延迟 (双向应用，实际 RTT 为 2 倍)
